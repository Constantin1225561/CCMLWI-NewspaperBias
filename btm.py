...
if __name__=="__main__":
    ...
    btm = BitermTopicModel(split[:10000], vocab_size)
    #btm.fit(num_iterations=250, num_topics=20,alpha=2.5,beta=0.01)
    btm.load_params('btm_params_100K.npz')
    for i, topic in enumerate(btm.get_topic_words(words_per_topic=20)):
        print("Topic {}: ".format(i), topic)
    topic_probs = btm.get_topics_for_docs(split[20000:20010])
    for i, probs in enumerate(topic_probs):
        print("\""+unique_headlines[20000+i]+"\": topic {}".format(np.argmax(probs)))
    btm.save_params('btm_params_100K.csv')